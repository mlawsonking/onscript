...
# 1. wait for every shard to have written its summary
while True:
    done = [n for n in CONGRESSES if (alexandria.ALEX / f"shard-{n}.json").exists()]
    if len(done) == len(CONGRESSES):
        break
...
print(f"[finalize] all {len(CONGRESSES)} shards present — merging …", flush=True)
res = alexandria.merge()
m = res["manifest"]
print(f"[finalize] merged: {m['ledger_entries']:,} ledger phrases, epoch {m['epoch']}", flush=True)

# 2. build the grounded chapter inputs (era-granular, coverage-gated)
inputs = chapters.build_era_inputs(res["ledger"], res["coverage"])
chapters.write_inputs(inputs)
sufficient = sum(1 for i in inputs if i["sufficient"])
print(f"[finalize] chapter inputs: {len(inputs)} eras, {sufficient} with adequate coverage "
      f"(the rest get honest code stubs)", flush=True)
