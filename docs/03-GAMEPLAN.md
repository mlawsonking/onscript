...
**"Explained over time" is era-granular, never per-day** (per-day retro-distillation ≈ $600+ one-time — dead). The v2 Archive release (§10) generates ~**300 monthly chapters + 13 per-Congress era essays** per party via the P2 voice machinery, inputs = ledger stats + code-extracted verbatim sentence windows (no per-statement extraction), verifier-checked like everything else.

**Generator policy (amended): recurring work = Actions + API; one-time capex = the cheapest capable tool, including Michael's own subscription and hardware.** The verifier is generator-agnostic (deterministic code over output text), so one-time generation is safely swappable:
- **Chapters/essays default: subscription-scripted Claude Code runs** — Phase 4 writes `scripts/generate-chapters.ps1` looping `claude -p` over chapter inputs, **chunked + idempotent by chapter id** (resumable across rate-limit windows; a few evenings). Marginal cost **$0**; model quality ≥ API Sonnet. Fallback if limits chafe or pinned-model provenance is preferred: Anthropic Batch API (≈$15–30 *per R6*, before Sep 1 pricing change). Every chapter records `generator + prompt_sha + verifier report` either way; the methodology page discloses the generator per artifact class.
- **Alexandria math on the 4080 (optional accelerator + one unlock):** local GPU embeddings turn the 670k-doc pass from CPU-hours into minutes; and a local 8–14B model can **topic-tag the entire historical corpus for $0** — previously unaffordable (≈$200+ of API, i.e., never planned) — unlocking 25-year topic-ownership and frame-war maps as Archive exhibits. One-time, on Michael's box, disclosed. Do **not** generate the chapters themselves locally: deadpan register is where small models are weakest, and the voice is the brand — words on Claude, math on the GPU.
- **The daily pipeline stays API-keyed** ($6–9/mo, §6.4) — a reliability decision, not a quality one: subscription capacity is shared with Michael's own usage and metered in rolling windows, and the streak must never depend on whether he had a heavy Claude week. The operating pipeline remains zero-babysit cloud; nothing recurring ever runs on a personal machine (charter constraint intact — one-time backfill jobs are capex, not runtime).
...
*Why this split is safe (the principle, for Phase 4):* citation integrity lives in the deterministic verifier and the receipts, not in the generator — **the generator is a commodity; the verifier is the product.** Neutrality likewise: identical prompts, thresholds, and audit apply regardless of which Claude produced the words, and the generator is disclosed per artifact.
...
**6.0 Scope note.** This section governs the *recurring daily pipeline* only; one-time backfill generation follows the §1.3 generator policy (subscription/local capex, API fallback).
