...
### 1.3 The backfill (do this in v1 — it's nearly free)
Pull `congress-press` history from **2025-01-03 (119th Congress seated) → today** (~47k releases); run the **local** engine only (normalize, boilerplate, n-grams, ledger, discipline index) — **zero LLM spend**; LLM distillation starts day 1 forward. Result: launch with 18 months of adoption curves, first-sayer records, and a party discipline time-series already computed. Every first-sayer claim carries the ledger epoch (`first seen in our corpus, which begins 2025-01-03`). The launch artifact (§9) comes from this backfill.
...
// phrase ledger entry (the compounding asset; epoch 2025-01-03)
...
4. **Launch artifact from the backfill:** "We measured 18 months of congressional speech. Here are the five biggest unison events of 2026 so far" — five adoption-curve cards + thread + day-one dashboard with history already alive.
...
**Obvious, now in plan:** (1) **Backfill the moat** — 18 months of history for $0 LLM spend; launch with the time-series instead of waiting for it (§1.3). (2) **Recess-proof spine as a *feature*** — October positioning: "Congress left Washington. The script went with them." (3) **Prompt transparency + nightly symmetry audit** as the neutrality answer nobody can argue with (§5.2–5.3). (4) **Leadership-origin tagging** — one roster join turns the Upstream Graph into a cheap v3 feature and every talking point gets a "leadership said it first" bit.
...
**Locked (do not relitigate in Phase 4):** OnScript naming + handle scheme · two-lane model + machine enforcement · daily-always cadence + governor · spine = press releases; Bluesky Lane 2; floor v2 · batch+cache+routing; only 3 prompt types in v1 · verifier is deterministic and blocking · raw→Release assets, derived→repo, ledger epoch 2025-01-03 · Astro/Vercel zero-server · prompts public + versioned · cut-line order (§1.2).

**Open knobs (defaults; deviate only on contact with reality, record it):** clustering method (default: MiniLM cosine + trigram-overlap union, threshold 0.80) · boilerplate DF percentile (default: top 0.5% party-corpus n-grams + regex list) · quiet-day threshold (default: <15 Lane 1 statements) · Bluesky poll depth (default: last 48h per author) · og-card renderer (default: headless Chrome; satori acceptable) · Astro vs plain static (default Astro).
