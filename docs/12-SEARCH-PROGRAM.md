# 12: The Search program (v1, Fable, 2026-07-15)

> This document defines the pre-registered hypothesis sweep of the 25-year archive. Fable authored it;
...
## §0 Ambition math (why it is sized the way it is)

Target: **≥24 CONFIRMED finding cards** = 1–2/month for 12–24 months of drip.
Catalog below: **47 pre-registered hypotheses.** Accurate expected survival, corpus-only families
~60% (they're descriptive), joined families ~40% (more confounds) → **expected ~22–28 confirmed.**
If few hypotheses survive, §3.S6 groups the null results into one piece: "we tested N folk theories
about congressional speech; K are false." A failed hypothesis can still produce a useful result.

**Tiers** (score at confirmation, not at ideation):
- **T1, high priority.** Novel, counterintuitive, or never measured. Requires one clear chart and strong evidence.
- **T2, solid.** Strong curiosity and a clear chart. Carries a monthly Appendix slot.
- **T3, supporting.** Ride-alongs and footnotes in larger pieces.
...
2. **Verdicts are exhaustive.** Every hypothesis ends in precisely one of:
...
5. **Symmetry because of the design.** Every metric computed identically for both parties, both numbers
...
12. **Failure-tested metrics.** Every metric function ships with unit tests INCLUDING a synthetic
    failure fixture: a fake corpus with a known injected confound (e.g. pure coverage growth) that the
    metric must NOT flag as a finding. House pattern; no metric touches real data until its
    failure fixture passes.
...
**S0.3 The metrics library** with failure fixtures (§1.12): `rate_per_1k`, `per_member_rate`,
...
**S0 acceptance:** harness and metrics covered by failure tests; reference tables committed with sources; a smoke
...
- **S1.4 · The Copy-Paste Caucus, verbatim/near-identical multi-member releases are a growing share
  ...
  direction agreeing. Risk: collapse-detector sensitivity is itself era-dependent, failure fixture
...
- **S3.3 · The Retirement Drift (H3 retrospective), the backtest.** Per-member centroid-distance
  ...
  reported honestly; REFUTE publishes too ("we tested the spooky thing; it's not there"). It is
...
- **S6.3** Grouped null result: if any family swept ≥5 hypotheses with ≤1 confirmation, that
...
items. They stay in HORIZON; the quarterly-pick rule governs them.
...
2. **Session shape:** pick up the next unfinished wave; build/extend harness + metrics with
   failure fixtures FIRST; run; record every verdict with its numbers in 13-SEARCH-LEDGER; write
...
3. **Published language:** "since 2013" for symmetric trends; "since 2001" permitted only for
   ...
   "in 2005 a talking point took a week…" framing in S1.1's blurb is retired, the accurate baseline is
   the 2013–2016 window.
4. Power floors (§1.6) unchanged; more early-window cells will correctly read UNDERPOWERED. It is
...
failure fixture before it runs.
