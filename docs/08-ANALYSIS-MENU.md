...
> The merged 25-year ledger turns the **private memo into a measurable public object**. For the
> first time, one synchronized, member-resolved, joint-collapsed record can trace the script's full
...
## Recommended pass (run on data in hand)
...
High value / now: Memo Detector · Forbidden Lexicon · Intraday Tick-Tock · Authors vs Vessels ·
Election Clock & October Hush · Phrase Obituaries & Euphemism Treadmill · Anatomy of the Ratchet ·
Backtest the Detector · The Distribution List (recurring member-sets = the memo's recipient list) ·
The Asymmetric Silence Board (ships v2 silence detector with the other party as baseline, zero
external deps).
High value / partial: Leadership Hub Test + Dead on Arrival · Assimilation Curve & Tenure Arc ·
Whip-Count Proxy (needs roll-calls) · Drift Before Departure (H3; needs pre-2025 denominators).
Medium / now: Phrase Lifespan/Zombies/Resurrections · Silent Minority / Caucus-Breakers · Vanishing
Common Tongue · Aisle-Jumpers & Migrations · Era Fingerprints (the Archive front page) · Sub-Caucus
Cartography & Connectors · Response Fatigue Curves · Does Losing Make You Louder · Locked in Step
(discipline-gap dynamics) · When the Machine Changed Gears (change-point) · A/B Testing on the Hill ·
The Hush (pre-blast quiet).
Partial / needs-source: Delegation Blocs · Cross-Chamber Current · Mobilization Latency Index ·
Committee as Coordination Channel (needs committee ingest + domain tagger).

## The one unblocker to do first

**Pre-2025 citation URL back-join** (small script, data already on `X:\onscript-data`): resolve
`first_seen` statement hashes / (bioguide, date) to URLs from the raw press-release files. This opens
the **citation gate for historical examples in ~a dozen analyses** (Vessels, Lexicon, Obituaries,
Migrations, Era Fingerprints, Silent Minority). Do it once, early.

Other acquisitions (all free / sanctioned): `unitedstates/congress-legislators` historical YAML
(per-Congress state/chamber/exit-reason) · per-Congress leadership rosters 113-118 · congress.gov
roll-calls (Whip-Count Proxy) · rule-defined SCOTUS/EO event calendar (Mobilization Latency) ·
pre-2025 raw-text normalization (per-member denominators) · GDELT (upgrades Silence Board to the
full news-diffed detector).
