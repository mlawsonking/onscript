# 14: Can we fill the 2001–2012 gap rigorously? (feasibility, 2026-07-15)

> **The question (Michael):** the press core source collapses before ~2013 and is single-party (congress 107
...
> than an accurate gap, it hides selection bias behind apparent completeness. So for every source the
...
symmetrically *because of the design*; three more provide real both-party signal for bounded sub-windows as
cross-checks; the naive "just scrape more press releases" routes either reproduce our exact bias or
fail aggregate symmetry. The 2001–2002 press *genre* stays an accurate gap unless one unprobed lead (LoC)
...
| **2** | **DCinbox, congressional e-newsletters** (Cormack) | **2010–2012** | Jan'10 112 D / 110 R (0.98); Jun'11 121 D / 206 R (0.59); Nov'12 107 D / 175 R (0.61); **0 unmapped bioguides.** | **VIABLE-WITH-CHANGES** | Independent both-party **cross-check**; its R-lean is *uncorrelated* with the core source's D-lean → a genuine validation instrument for 2011–12, not filler. Keyless bulk CSVs (141 monthly; dumps end 2021-09 → mirror on first ingest). |
...
wrong-genre. **Keep the accurate gap over any of these.** (Also considered + rejected: GovInfo CHRG
hearings (genre), congressional tweet archives (2017+, dead predecessor's lane), university member-papers
(asymmetric, non-machine).)

## The accurate build, instrument-by-window (every lane labeled, schema-tagged, never silently merged)

- **Press core source, 2013–2026** (`source=press`, primary, merge-safe): the existing dwillis two-party
...
- **Academic archive lane, 2005–2012** (`source=academic_archive`, separate; Wang truncated ≤2012-12-31
  to not double-count the clean core source): cross-checks and existence lookups only, **never** census numbers.
- **Wayback per-member lane** (`source=wayback`, coverage-gated): opportunistic single-member color; never
  aggregated.
- **2001–2002 press-genre = accurate, disclosed gap**, unless the **LoC** probe (Dec 2002+) clears text
...
Default posture: *fill only where symmetry is proven; otherwise accurate gap.* Enforced **in code**, not
just docs.
...
6. **GENRE ISOLATION, the sharpest gate:** separate lanes are necessary but NOT sufficient. The actual
   trap is a sentence like *"coordination rose from 2008 to 2015"* that silently compares CREC floor
   speech (2008) to the press core source (2015), a genre confound wearing a temporal-trend costume.
...
Ran the #1 lead directly. **Coverage is the actual symmetry win:** the collection's keyless JSON API
...
*before* ingesting, which is precisely what generic Wayback cannot offer. Each item carries the historical
...
4. **Keep 2001–2002 press-genre as an accurate, disclosed gap** unless LoC clears it.

**Bottom line:** the "25-year symmetric archive" is real after all, but as a **Congressional Record
instrument**, clearly labeled and never merged into the press core source, with Extensions of Remarks as the
