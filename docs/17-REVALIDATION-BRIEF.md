# 17: Revalidation brief (Fable, 2026-07-17, binding)

**Who runs this: Opus, in the main tree, after reading CLAUDE.md.** It is build-order item (3):
...
- **Runnable now** (direct `iter_statements(lane=...)` or rebuilt text_features): all S2 items;
  ...
  **because of the design**, re-affirm and say so in its row); S4 items (fix `wave_s4._collect`'s
...
- Classify every one of the 34 as: RUNNABLE-WITHIN-LANE · NEEDS-RESCOPE (construct requires the
  cross-seam span; re-register within-lane or retire) · LANE-IMPOSSIBLE (retire, accurate row) ·
...
1. **S2.3** (what-losing-sounds-like), the flagship reversal whose kill may be plumbing. Its own
   ...
   the scraped lane. It is the single highest-value re-validation in the program.
2. **S1.9** (self-audit), quick re-affirm, lane-clean because of the design; its CONFIRMED either
