# 13: Search verdict ledger (append-only; Opus maintains)
...
Status markers: ✅ confirmed; ⚠ caution or review gate; 🔬 source validation required.

> One row per pre-registered hypothesis in `docs/12-SEARCH-PROGRAM.md`. Every hypothesis ends in
> precisely one verdict: CONFIRMED · REFUTED · UNDERPOWERED · ARTIFACT (confound named) · BLOCKED.
...
and accurate; the drip value is carried by CONFIRMED S1.9 + the emergent narratives/reversals.
...
- `raw/congress-press`, **303 monthly JSONL, 2001-01 → 2026-07** (the confirmed-complete ground
  ...
  out ("Democrat"/"Republican"). **It is the Search substrate.**
...
**Critical conclusion → Amendment A1 (see 12 §6):** the analyzable *symmetric* span is
...
**S0.3 metrics library:** built + tested with failure fixtures (`pipeline/search/metrics.py`,
...
- **S1.1 / S1.3 (2026-07-15):** the two flagship genealogy metrics returned a naive CONFIRMED that was
  ...
  caught it before it became a false "the memo industrialized" headline. It is the discipline paying
...
- **S1.2 / S1.5 / S1.7 (2026-07-15):** three trend-guesses refuted, but *into* real findings, not
  ...
  failed hypotheses are producing useful inversions, the type of counterintuitive result the Appendix
...
**S2.11 Euphemism Genealogies** | **DESCRIPTIVE** | 12 famous pairs; e.g. "climate change" D 34,581 / R 3,964; "death tax" D 32 / R 2,348; "obamacare" D 2,095 / R 36,808 | 2026-07-16 | Per-pair party split is the artifact (each pair its own mini-card). Verify agent reproduced counts precisely, 24/24 label-sides hold in both halves. (Prose caveat: don't say Luntz "coined" climate change, he recommended an existing IPCC-1988 term.)
...
inversion with all four series shown. (It is why the graveyard is not waste, it composed a headline.)
...
| S3.7 | The Safe-Seat Vessel Test | **BLOCKED** (source; **cheapest unblock in the wave**) | `elections.json` has **no margin of victory** (it is 7 computed election dates); **no MoV table is mirrored anywhere** in `data/**` or `X:\onscript-data\**`. **The independent variable has zero rows** | 2026-07-16 | **One acquisition turns this into a real GO:** MIT Election Lab / FEC results → `bioguide × cycle → margin` JSON, same offline-parse-and-commit pattern as `committee-names.json`. Yields **~90–100 members per quintile**, genuine power, and the spec is accurate either way ("either direction confirms something; flat REFUTES both folk theories"). **It is the wave's one recoverable hypothesis and the highest-value item to unblock.** |
...
subagent did not.** It is the fourth §4.5 reversal of the program (after S1.10, S2.3, and the
...
the remaining 14** hypotheses to confirm. There is no recovery path, it is not "behind pace," it is
**an impossible remainder.** At the observed 6.1% rate the 47-hypothesis catalog terminates around
**3 CONFIRMED**; at a generous 15% for the un-run remainder, **4–5**. The accurate ceiling is **4–6, not
22–28.** The §0 survival assumptions were off by roughly **5×**, and the error is now large enough that
leaving the number in the doc is itself a small disaccuratey.

**But S3 does not refute the premise the way the number implies, and the distinction is critical.**
...
1. **The 2013 cliff is not a caveat, it is the boundary of the system.** Amendment A1 said the
...
precisely), **S4.2 is 7 shutdowns**, plus **2 CONFIRMED** (S1.9, S2.9), **the Great Intensification**
...
### The accurate goal (recommend replacing docs/12 §0)
...
1. **Amend §0 now, before Wave S4/S5 pre-registration inherits the same fantasy.** It is Fable's call
...
> different window set (the actual table merges to 5 events per its own note fields).
...
| S4.5 | (winner/loser-party language) | **BLOCKED** |, | 2026-07-16 | `elections.json` carries **seven dates and nothing else**, no winners, no losers, no margins. The spec's entire dependent variable is absent from its only join key. **It is the S3.7 failure verbatim.** Independently, the "13 cycles" gate is unreachable: 7 dates (2014–2026), 2026 is in the future (measured D=0/R=0) → 6 usable. Re-speccable as a 6-cycle DESCRIPTIVE using `chambers-control.json` as a coarse, disclosed winner proxy, **only as a new pre-registration, not a rescue.** |
...
- **S4.4 (2026-07-16), the model entry.** A famous folk theory, pre-registered and hashed *before*
  ...
  placebo lexicon 0.98×). Both CI upper bounds fall below the gate, so it is an **equivalence
  result**: the Dump is bounded away from its claimed size, not merely undetected. Three skeptics
  found nothing. It is what docs/05 §2.5 meant by the graveyard being the product.
- **S4.4's design lesson belongs in docs/12 before Wave S4 is specced further.** S4.4 needed **no
  ...
  **Event-anchored phrase selection manufactures nomenclature convergence because of the design.**
- **S4.2 (2026-07-16), the fourth reversal, and the most instructive.** Its `accurate_note` contained
  ...
  computational, the system is accurate, reproducible, symmetric (D/R regex skeletons byte-identical
  after token substitution), and its multi-speaker gate was critical (dropped 31 hits in 2018-19 D
...
## Program-level findings that outrank any new measurement
...
- **The 2021 coverage collapse is undocumented and is gating the program's primary robustness test.**
  ...
  sits **precisely on the A/B half boundary of BOTH `scotus-landmarks.json` and `shutdowns.json`** ,
...
**1. CONFIRMED-counting is dead, and S4 is the proof rather than the cause.** 2 of 34 (5.9%) with zero from two consecutive waves. But the wave's single best artifact, S4.4, is a **REFUTED**. A hashed pre-registration, 674,970 statements, a 1.5× gate missed at 0.85×, an equivalence result (both CI uppers below the gate), and a **positive control at 2.10×** proving the system could have found the effect. Under CONFIRMED-counting that is a zero. Under card-counting it is the wave's flagship. The metric change is not a lowered bar, it is the correct bar, and S4.4 is the worked example.

**2. The "~45+ cards in hand" number does not survive contact with disk.** `data/derived/findings/` contains **8 files**: GREAT-INTENSIFICATION, S1.4, S1.9, S1.10, S2.1, S2.3, S2.4, S2.9. The 45+ is a count of *candidates* claimed across sessions, per-case series, reversal candidates, per-pair mini-cards (S2.11's 12 euphemism pairs, S4.1's 20 rulings), not written, verifier-passed, publishable cards. **Re-baselining onto a number that is 5.6× the on-disk reality would import precisely the failure mode S4.2 just died of: headlining a statistic other than the one that was actually validated.** The accurate re-baseline needs the audit first: how many of the 45+ survive a nomenclature audit, a denominator check, and a placebo against *their own headline statistic*? S4 says the attrition is severe, S4.1 alone loses its entire 20-card top-phrases arm to a ~97% naming fraction, and S4.2 loses all of its.
...
## Warning: provenance-seam defect (2026-07-16). Read before trusting any verdict above.
...
refuted)**; one re-derived every critical number directly from the 303 mirror files, deliberately
bypassing `harness.iter_statements` so no harness quirk could propagate.

### Effect on split-halves, the program's primary robustness control
...
- **S4.7 (January 6), a SIGN INVERSION, not an exaggeration.** Raw: **-69.9%** -> *"muted
  ...
  **+75.5%**. Fixed-cohort: January 2021 is a local **maximum**. The 70% "drop" is the ProPublica
  import ending three days before. It is the closest OnScript has come to publishing a damaging
...
### Remedy: lane isolation in code, not normalization
...
term, while the CREC data on disk is precisely 2001-2008 (the presidency it omits), and
`chambers-control.json` covers only 113-119, so 2 of SD.3's 5 named flips have no control coding.
Both are trivial offline edits; both are critical.

**A p-hacking hole, found by the gate-reachability audit docs/12 never got:** S5.2's floor is directly
">=Floor per cell", no number was ever pre-registered.

---

## Revalidation within one lane (2026-07-17, Opus Session 18; docs/17 brief). Append-only; supersedes named.
...
lane split sums precisely to the pre-seam totals (concern-ladder 39,249 / 5,808 / 650 / 2,084), which is
...
- `pipeline/search/wave_s1.py`: `s1_4_verbatim` and `s1_10_bipartisan_season` gained `lane=`;
  `s1_9_self_audit` gained `lane=` (144 legacy 2021-01-03 records confirmed non-critical);
...
1. **"S1.9 is lane-clean because of the design" is 99.6%, not 100%.** Congress 117 contains 144 ProPublica
   records dated precisely 2021-01-03 (the import's last day == the 117th's first day). The exclusion is
   a measured no-op (verdict, weeks-matched, and D>R count all identical), so the finding is unmoved ,
   but "because of the design" overstated it; it is "clean after excluding 144 records, which changes
   nothing."
...
## Shard-lanes revalidation: eleven BLOCKED-ON-SHARDS items within one lane (2026-07-17, Opus Session 19; docs/18 brief). Append-only; supersedes named.
...
| S1.6 | **UNDERPOWERED** (lane cost) | only 1 election cycle per half in the scraped lane (2022 in A, 2024 in B); tally 1/1 every cell (<2 floor) | the accurate cost of isolation, the electoral-cycle gate needs cycles from both eras | ≥2 cycles/half/party |
...
**Scraped-lane read:** 5 REFUTED-stands, 2 ARTIFACT-stands (S1.1/S1.3, proving the sawtooth is NOT the seam), 2 ARTIFACT-coverage (S1.1′/S1.3′, the intensification story is absent within 2021-26, so it was a propublica-era effect), 2 UNDERPOWERED (S1.6 electoral cycles, S1.4-proper congress-split, both accurate lane-isolation costs, not failures). **Zero verdicts flipped toward a false positive.**
...
the system that measured it, and it stopped at the seam.** S1.1′ and S1.3′ were REFUTED across the
...
the within-lane halves were PRE-REGISTERED in docs/18 §5 (so it is not a p-hacked reversal, the
...
| UNDERPOWERED (accurate lane cost) | S1.6 (scraped), S1.4-proper (both lanes) |
...
## New measurements (2026-07-19, Opus Session 26): S1.12 run within one lane; S3.7 registered and blocked
...
(0.82/0.89/1.61/0.95 ≈ identical). **One variant points differently and is the accurate nuance:**
...
could be a card.) Publishable NULL, docs/20 graveyard/methods shelf; symmetric because of the design (one
...
concordance score.** This floor is precisely why the Senate half is NOT run alone: ~100 senators total,
...
**#177 closed → the House `.tab` is local; S3.7 ran precisely as registered, no knob, floor, or rescope
...
5 surname+date / **0 party-relaxed**, independents King & Sanders matched precisely). The **2 unmatched are
the GA 2020/21 dual-runoff** (Perdue lost his runoff → no term; Ossoff's runoff is coded to an odd year) ,
an accurate ~1-senator gap, not a defect. MoV ∈ [0,1], median 0.272, 65 uncontested (=1.0). Full unmatched
...
- **The 0.20 effect-size gate earned its keep:** scraped·B is *statistically* significant (p=0.012 at
  ...
  a meaningless correlation is precisely what a p-only rule would have mis-sold as CONFIRM. Accurate nuance,
...
- **The rate is match-strictness sensitive, so the RANGE is the accurate finding:** non-conversion is
...
- **Party comparative gate (registered = gap > summed 95% CI half-widths):** overall **D 8.6% vs R 6.8%**
  ...
  SYMMETRIC instrument (identical lexicon/match/threshold, party-blind because of the design; Art. IV protected).
...
- **Verdict:** a stark, well-powered rate-report **card** (docs/12 S5.2 "T1 if the number is stark").
  ...
  S1.9/S2.9/Intensification, publication is his act, never a session's). **It is the program's first
...
## HX registration wave (docs/05 §3): substrate audit and HX.8 measurement (launch-eve Search lane, 2026-07-20, Opus)
...
Both are methods/transparency-shelf descriptives (docs/20), symmetric because of the design. Re-runnable:
...
- **Question:** does a party sustain its coordinated talking points LONGER when it holds institutional
  power (House majority) than when it does not? **Symmetric because of the design**, House control flips
...
Symmetric because of the design (House control flips between parties across congresses).
...
- **⚠ Disclosed confound, propublica's majority is party-collinear.** House control was R for 3 of that
  ...
  separated. **The scraped lane is the critical evidence:** its House control is genuinely mixed
...
majority-vs-minority persistence. The partition reproduces HX.4's cells **precisely** (propublica maj
...
- **Question:** within each party, does OPPOSITION (condemnation)-framed language get REUSED (echoed
  across the party's own statements) more than CELEBRATION-framed language? Symmetric because of the design
...
- **⚠ Substrate audit killed two naive metrics before the freeze (marginals only, no effect measured):**
  ...
  by furniture, not talking points. *(It is itself a ledger-worthy caution: the peak≥15 set is not a
...
- **Placebo (docs/12 L3, S4.2's law, the EXACT statistic):** within-valence split (sort each lexicon,
  ...
  a bootstrap 95% CI (B=500, frozen seed) on the actual gap and each placebo gap. The **placebo is the
  effect-size yardstick**, the actual gap must clear significance AND exceed the within-valence placebo
...
effect in every one**, so it is a lexical-split artifact, not a valence effect. The placebo did precisely
...
- **Why it's an artifact:** the `repeat_rate` (share of a class's 4–6-gram phrase-mass echoed by ≥1 other
  ...
  homogeneous and therefore more self-echoing. The actual opp>cel gap is a byproduct of the con/cel lexicons
...
- **⚠ Disclosed implementation deviation (benign):** the frozen registration specified `random.Random(0)`
  ...
  frozen metric, size-matching (n=min), placebo, B=500, and verdict rule **precisely**; only the CI endpoints
