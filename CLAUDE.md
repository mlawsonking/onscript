...
| 3 | **Fable** | Gameplan — final v1 scope (must ship in one weekend), architecture, schemas, pipeline stages, distillation prompts, voice guide, launch sequence, phased roadmap with acceptance criteria. | `docs/03-GAMEPLAN.md` | ⬅️ NEXT |
...
- **Phase 2 complete** (Opus, 2026-07-10): `docs/02-RESEARCH.md` written — every assumption R1–R11 validated against live primary sources and adversarially re-checked; every §3 feature marked. Headline results that reshape v1: **(1) The spine flips** — floor speech (R1) is a citation leg, *not* the spine (procedural-heavy + Congress in recess all of October 2026); the daily spine is **member press releases** (`dwillis/congress-press` corpus, R2) **+ Bluesky** (R3). **(2) X is DEAD** (R4 — pay-per-use since Feb 2026, 40–120× over budget); ingest from open sources, post on Bluesky + dashboard, manual X cross-post only. **(3) "Party Lines" is DEAD as a name** (R11 — active US-politics podcast collision) → rename to **OnScript**. **(4)** R5 news baseline = **GDELT 2.0** (free, redistributable). **(5)** $10/mo fits only with batch+cache+model-routing and likely in-session-only cadence (R6); public repo → free compute + Release-asset corpus store (R7). **The #1 strategic risk:** X-dead + Bluesky-94%-Democratic means press releases are the *only* symmetric two-party source, so neutrality-by-construction now rests on one volunteer-maintained scraper — mirror it. No §3 feature is dead.
- **Next: Phase 3 (Fable).** Read the handoff note at the top of `docs/02-RESEARCH.md`, then produce `docs/03-GAMEPLAN.md`: the weekend-sized v1 scope built around the corrected spine (press releases + Bluesky), the OnScript name, the batch-and-cache cost discipline, GovInfo-primary ingestion architecture, and the neutrality defense for asymmetric sources — with schemas, pipeline stages, distillation prompts, voice guide, launch sequence, and phased roadmap with acceptance criteria. Open questions research couldn't settle are in §5 of the research doc.
