...
- **Next: Phase 4 (Opus).** Read the handoff note at the top of `docs/03-GAMEPLAN.md`; build §1 exactly (v1 acceptance criteria §1.4 including both kill-tests); locked decisions in §13 are not to be relitigated. One human errand gates launch: Michael buys domains/handles (§7.3, ~$30 one-time). **Phase 4 spans multiple sessions:** maintain `docs/04-BUILDLOG.md` (per-session progress against §1.4, deviations under §13 rules with rationale) so any fresh session resumes without re-deriving state; the phase is done when §1.4 passes in full, not when code exists.
