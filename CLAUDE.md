...
- **Phase 4 in progress (Opus, multi-session).** Running state lives in `docs/04-BUILDLOG.md` (per-session progress against §1.4, sanctioned §13 deviations). Build §1 exactly; §13 locked decisions are not relitigated. The phase is done when §1.4 passes in full, not when code exists.
  ...
  - **Next sessions:** LLM layer (P1/P2 batch + blocking verifier over live output, needs `ANTHROPIC_API_KEY`), RUN A/RUN B Actions workflows + ops (governor, ntfy, symmetry report, Release upload), Astro site, kill-tests, then the 3-consecutive-unattended-runs gate. Human gate (§7.3/§9): Michael registers domains/handles, creates the public repo + Bluesky accounts + Actions secrets.
