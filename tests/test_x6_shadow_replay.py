...
def test_committed_days_run_end_to_end_and_emit_the_full_comparison_report():
    ...
    expected_days = len(shadow_replay._complete_days(DAYS))
    assert report["mode"] == "dry_run"
    assert report["window"]["complete_days"] == expected_days
    assert report["window"]["party_days"] == expected_days * len(config.COMPOSITE_PARTIES)
    assert report["minimums"] == {"complete_days": 60, "party_days": 200}
    assert set(report["prompt_inventory"]) == {"P2", "P3"}
    assert set(report["candidate"]["guard_violation_party_days"]) == {
        "unit_mixing", "quote_extension", "topic_label_assertion",
        "multi_claim_sentence", "sentence_mapping_mismatch",
    }
    assert report["candidate"]["fallback_rate_denominator"] == report["window"]["party_days"]
    ...
    assert len(report["party_day_results"]) == report["window"]["party_days"]
...
def test_dry_run_never_calls_the_api_even_when_a_key_exists():
    ...
    try:
        report = shadow_replay.run(DAYS, live=False, allow_api_spend=True, limit=2)
    finally:
...
def test_documented_cli_runs_dry_and_emits_json_without_a_key():
    completed = subprocess.run(
        [sys.executable, "scripts/shadow_replay.py", "--days-dir", str(DAYS), "--limit", "2"],
        cwd=ROOT, check=True, capture_output=True, text=True,
    )
    report = json.loads(completed.stdout)
    assert report["mode"] == "dry_run"
    assert report["window"]["complete_days"] == 2
    assert report["window"]["party_days"] == 4
